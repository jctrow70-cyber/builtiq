...
## BIQ-0023 - dd/mm/yy Date Format and Monday Week Start
...
Switched user-facing dates to **dd/mm/yy** and aligned program and dashboard weeks to **Monday–Sunday** (Sunday is the last day of the week).

### Purpose

Users wanted European-style dates and calendar weeks that start on Monday instead of rolling 7-day windows or ISO-style `YYYY-MM-DD` display.

### Changes

- **Display format** — `formatDisplayDate` / `parseDisplayDate` in `programCalendar.ts`; Progress, Dashboard, Training, and team views show dd/mm/yy
- **Date inputs** — new `DateInput` component accepts dd/mm/yy while storing `YYYY-MM-DD` internally for Supabase
...
1. Open Training — Date and Program start fields accept **dd/mm/yy** (e.g. `13/07/26`)
2. Confirm week selector shows ranges like `07/07/26 – 13/07/26` (Mon–Sun)
3. Confirm day tabs show dd/mm/yy next to each workout day
...
6. Progress — history dates display as dd/mm/yy
...
- Native browser locale is no longer used for date pickers; users type dd/mm/yy manually
- Existing `start_date` values are unchanged in the database; week math normalizes to Monday via `programWeekAnchor`

### Recommended commit message

```text
BIQ-0023 Use dd/mm/yy dates and Monday–Sunday week alignment
```
