...
/** Seven days ending on endDateYmd (inclusive), oldest first. */
export function buildSevenDayNutritionSummary(
  entries: EntryRow[],
  goals: NutritionGoals,
  endDateYmd: string
): WeeklyNutritionSummary {
  const sunday = String(endDateYmd || '').slice(0, 10);
  const monday = addDaysYmd(sunday, -6);
