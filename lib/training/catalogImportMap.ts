import { inferExerciseType, EXERCISE_TYPES, type ExerciseType } from './exerciseTypes';
import { normalizeMovementPattern } from './exerciseIntelligence';
import type { ExternalExerciseRecord, MappedCatalogRow } from './catalogImportTypes';
...
function buildMuscleTargets(primary: string, secondaries: string[]) {
  ...
  if (secondaries.length) {
    ...
    const remainder = (secondaryPct || 0) - each * secondaries.length;
