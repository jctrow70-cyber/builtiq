...
1. **Program Design overhaul (BIQ-0136)** — planning is separate from Training execution (Decision 030). After generate, Programs shows the built workouts (BIQ-0157). AI designs the week inside science constraints (BIQ-0161 / BIQ-0163). Intake is structured + optional notes (BIQ-0165) and now actually changes the saved week (BIQ-0166). Remaining: cardio/mobility days in intake, chat updating structured fields, occurrence-level recurring edits, catalog pick when adding exercises, program edit-from-prompt, and progression from logged performance.
