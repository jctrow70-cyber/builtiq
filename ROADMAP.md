...
<<<<<<< HEAD
1. **Program Design overhaul (BIQ-0136)** — planning is separate from Training execution (Decision 030). After generate, Programs shows the built workouts (BIQ-0157). AI designs the week inside science constraints (BIQ-0161 / BIQ-0163). Intake is structured + optional notes (BIQ-0165) and now actually changes the saved week (BIQ-0166). Programs edits use the Training catalog editor (BIQ-0167), including ↑ ↓ reorder (BIQ-0169). Group members follow the live group template so they see owner/editor edits (BIQ-0168). Training calendar repeats can cover multiple weekdays; programs default to training-only with an optional all-inclusive plan for group push (BIQ-0170). Remaining: cardio/mobility days in intake, chat updating structured fields, occurrence-level recurring edits, program edit-from-prompt, and progression from logged performance.
=======
1. **Program Design overhaul (BIQ-0136)** — planning is separate from Training execution (Decision 030). After generate, Programs shows the built workouts (BIQ-0157). AI designs the week inside science constraints (BIQ-0161 / BIQ-0163). Intake is structured + optional notes (BIQ-0165) and now actually changes the saved week (BIQ-0166). Programs edits use the Training catalog editor (BIQ-0167), including ↑ ↓ reorder (BIQ-0169). Group members follow the live group template so they see owner/editor edits (BIQ-0168). Custom cycle length is respected on generate (BIQ-0170). Same-day deadlift stacking is blocked (BIQ-0171). Remaining: cardio/mobility days in intake, chat updating structured fields, occurrence-level recurring edits, program edit-from-prompt, and progression from logged performance.
