...
## Decision 027 - Team Training Coach Platform Architecture
...
Team Training is split into two experiences on the same workout engine:

- **Personal Training** = athlete view (own logging, own progress)
- **Team Training** = coach view for owners/editors (dashboard, roster, athlete dashboards, assignments)

Athletes on Team Training see only their own plan toggle and workout entry — not the full team roster. Coach metrics, alerts, and analytics plug into `lib/training/teamCoach/` so future KPIs do not require a redesign.

### Reason

Mirroring Personal Training for coaches does not scale to athletic programs. Coaches need roster status, compliance, assignment workflows, and per-athlete performance views. Athletes need privacy and a focused logging experience.

### Alternatives Considered

- Single unified Team tab for all roles — rejected; athletes saw coach roster UI
- Separate coach app — rejected for MVP; same codebase with role-based views is sufficient
- Implement full analytics now — deferred; architecture hooks only in BIQ-0027

### Impact

- `lib/training/teamCoach/` domain module (permissions, program resolution, metrics, status)
- Coach UI components: dashboard, roster, athlete dashboard, assignment panel
- `TeamAthleteView` for member role
- Future: team volume graphs, compliance ranking, coach-defined KPIs attach to `coachMetrics.ts`
