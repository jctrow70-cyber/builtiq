...
import { CATALOG_SOURCES, catalogSourceLabel, normalizeCatalogSources, type CatalogSourceId } from '../lib/training/catalogSources';
...
const emptyAddPanelFilters=()=>({muscle:'',equipment:'',exerciseType:'',guidesOnly:true});
...
export default function Page(){
 ...
 async function saveProfile(markComplete=true){
  ...
  const{error}=await supabase.from('st_profiles').upsert({
   ...
   catalog_sources:normalizeCatalogSources(profileDraft.catalog_sources),
...
const weekWorkouts=(program?.st_workouts||[]).filter((w:any)=>w.week===week).sort((a:any,b:any)=>a.day_order-b.day_order);
 ...
 const builtinCatalog=useMemo(()=>builtinCatalogItems(catalog,normalizeCatalogSources(profileDraft.catalog_sources)),[catalog,profileDraft.catalog_sources]);
 ...
 const catalogSourceChips=(list:CatalogSourceId[],onToggle:(id:CatalogSourceId)=>void)=><><label>Exercise libraries</label><p className="muted">Choose which exercise databases appear when adding or searching exercises. Enable <b>Guided Library</b> for GIF demos and instructions (run import first). Essentials + Basic are text-only unless imported.</p><div className="focus-muscle-grid equipment-grid">{CATALOG_SOURCES.map((o)=><button type="button" key={o.id} className={`focus-chip${list.includes(o.id)?' active':''}`} onClick={()=>onToggle(o.id)} title={o.description}>{o.label}</button>)}</div>{list.length>0&&<p className="muted">Active: {list.map((id)=>catalogSourceLabel(id)).join(', ')}</p>}</>;
 ...
 const profileFields=(compact=false)=><>
  ...
  {catalogSourceChips(normalizeCatalogSources(profileDraft.catalog_sources),(id)=>setProfileDraft({...profileDraft,catalog_sources:toggleCatalogSourceList(normalizeCatalogSources(profileDraft.catalog_sources),id)}))}
