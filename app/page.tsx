...
const weekWorkouts=(program?.st_workouts||[]).filter((w:any)=>w.week===week).sort((a:any,b:any)=>a.day_order-b.day_order);
 ...
 <main className="main page-main">
  ...
  {appNav==='Training'&&<section>
    ...
    {!viewingMember&&<div className="applybox"><label>When changing workout structure, apply edits to:</label><select value={applyScope} onChange={e=>setApplyScope(e.target.value as any)}><option value="future">This week and all future weeks</option><option value="current">This workout only</option></select></div>}
    ...
    {workout&&SECTIONS.map((sec:any)=>{
      ...
      const batch=supersetBatch[sec.id];
      ...
      {canEdit()&&<div className="exercise-add-row">{catalogError&&<p className="muted" style={{color:'#f87171',gridColumn:'1/-1'}}>Catalog unavailable: {catalogError}</p>}<div className="typeahead-wrap"><input className="typeahead-input" placeholder="Search name or muscle (e.g. chest)" value={query} onChange={e=>{setCatalogQuery({...catalogQuery,[sec.id]:e.target.value});setTypeaheadOpen({...typeaheadOpen,[sec.id]:true});}} onFocus={()=>setTypeaheadOpen({...typeaheadOpen,[sec.id]:true})} onBlur={()=>setTimeout(()=>setTypeaheadOpen({...typeaheadOpen,[sec.id]:false}),150)}/>{query.trim()&&typeaheadOpen[sec.id]&&<div className="typeahead-menu">{results.length?results.map((item:any)=><button type="button" key={item.id} className="typeahead-item" onMouseDown={e=>e.preventDefault()} onClick={()=>addExerciseFromCatalog(item,sec.id)}><b>{item.name}</b><span className="muted">{item.muscle_group||'Muscle'}</span></button>):<div className="typeahead-empty muted">No matches</div>}</div>}</div><label className="superset-check" title="Add next exercises to same superset"><input type="checkbox" checked={!!supersetAdd[sec.id]} onChange={e=>{const on=e.target.checked;setSupersetAdd({...supersetAdd,[sec.id]:on});if(!on)setSupersetBatch({...supersetBatch,[sec.id]:null});}}/>SS</label>{batch&&<span className="badge superset-badge">{batch.count}/3</span>}<button type="button" className="btn small secondary" onClick={()=>{setShowCustomForm({...showCustomForm,[sec.id]:!customOpen});setCustomDraft({...customDraft,category:sec.id});}}>+ Custom</button></div>}{canEdit()&&customOpen&&<div className="catalog-edit-grid" style={{marginTop:8}}><input value={customSectionDraft.name} onChange={e=>setCustomDraft({...customSectionDraft,name:e.target.value})} placeholder="Exercise name"/><select value={customSectionDraft.category} onChange={e=>setCustomDraft({...customSectionDraft,category:e.target.value})}><option value="warmup">Warmup</option><option value="strength">Strength</option><option value="mobility">Mobility</option><option value="plyometric">Plyometric</option><option value="other">Other</option></select><input value={customSectionDraft.muscle_group} onChange={e=>setCustomDraft({...customSectionDraft,muscle_group:e.target.value})} placeholder="Muscle group"/><button className="btn small green" onClick={()=>createCustomExercise(sec.id)}>Save & add</button></div>}
