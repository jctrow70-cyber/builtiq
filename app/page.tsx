...
const weekWorkouts=(program?.st_workouts||[]).filter((w:any)=>w.week===week).sort((a:any,b:any)=>a.day_order-b.day_order);
 ...
 return <><header className="header"><div><div className="brand">Built<span>IQ</span></div><div className="muted">{displayName} · {mode==='team'?(team?.name||'Team'):'Personal'} · {canEdit()?'can edit plan':'log only'}</div></div><button className="btn secondary" onClick={()=>supabase.auth.signOut()}>Sign Out</button></header>
 <div className="shell"><aside className="panel">
  ...
  {mode==='team'&&<div className="card"><label>Team</label><select value={team?.id||''} onChange={e=>setTeam(teams.find((t:any)=>t.id===e.target.value))}><option value="">Select</option>{teams.map((t:any)=><option key={t.id} value={t.id}>{t.name} · {t.my_role}</option>)}</select><div className="actions" style={{marginTop:8}}><button className="btn small secondary" onClick={createTeam}>Create</button><button className="btn small secondary" onClick={joinTeam}>Join</button></div>{team&&<p className="muted">Invite: {team.invite_code}</p>}</div>}
