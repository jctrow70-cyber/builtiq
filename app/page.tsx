...
import TrainingGroupRow from './components/training/TrainingGroupRow';
import TrainingPlanSummary from './components/training/TrainingPlanSummary';
...
export default function Page(){
 ...
 async function loadTeams(){if(!session?.user)return; const{data}=await supabase.from('st_team_members').select('*, st_teams(*)').eq('user_id',session.user.id).eq('status','active'); const ts=(data||[]).map((m:any)=>m.st_teams&&!m.st_teams.is_archived?{...m.st_teams,my_role:m.role,training_source:m.training_source||'team',membership_id:m.id,is_active_participant:m.is_active_participant!==false}:null).filter(Boolean); setTeams(ts); setSelectedTeamId((prev)=>prev&&ts.some((t:any)=>t.id===prev)?prev:ts[0]?.id||null);}
 async function createTeam(name?:string){const groupName=(name||'').trim()||'My Team'; const code=makeInviteCode(); const{data:t,error}=await supabase.from('st_teams').insert({name:groupName,invite_code:code,owner_user_id:session.user.id}).select().single(); if(error)throw new Error(error.message); const{error:me}=await supabase.from('st_team_members').insert({team_id:t.id,user_id:session.user.id,display_name:displayName,role:'owner'}); if(me)throw new Error(me.message); await loadTeams(); setMode('team'); setSelectedTeamId(t.id); setAppNav('Groups');}
 async function joinTeam(code?:string){const inviteCode=(code||'').trim(); if(!inviteCode)throw new Error('Invite code required'); const{data:t,error}=await supabase.rpc('st_join_team_by_invite',{p_invite_code:inviteCode,p_display_name:displayName}); if(error||!t)throw new Error(error?.message||'Team not found'); await loadTeams(); setMode('team'); setSelectedTeamId(t.id); setAppNav('Groups');}
 ...
 function applyScheduleOption(opt:any){
  ...
  setDayEmphasis(opt.id==='opt_requested'&&opt.day_emphasis?opt.day_emphasis:{});
  ...
  {appNav==='Training'&&<section>
    ...
    {trainingSubNav==='personal'&&!viewingMember&&!activeAssignedRecipient&&<>
      ...
      <TrainingPlanSummary
        ...
        canManage={canManageTrainingProgram}
        onManageProgram={openManageProgram}
