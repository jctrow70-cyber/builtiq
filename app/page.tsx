...
const weekWorkouts=(program?.st_workouts||[]).filter((w:any)=>w.week===week).sort((a:any,b:any)=>a.day_order-b.day_order);
 ...
 const catalogSourceChips=(list:CatalogSourceId[],onToggle:(id:CatalogSourceId)=>void)=><><label>Exercise libraries</label><p className="muted">Choose which exercise databases appear when adding or searching exercises. Enable <b>Guided Library</b> for GIF demos and form instructions.</p><div className="focus-muscle-grid equipment-grid">{CATALOG_SOURCES.map((o)=><button type="button" key={o.id} className={`focus-chip${list.includes(o.id)?' active':''}`} onClick={()=>onToggle(o.id)} title={o.description}>{o.label}</button>)}</div>{list.length>0&&<p className="muted">Active: {list.map((id)=>catalogSourceLabel(id)).join(', ')}</p>}</>;
