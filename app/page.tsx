...
const weekWorkouts=(program?.st_workouts||[]).filter((w:any)=>w.week===week).sort((a:any,b:any)=>a.day_order-b.day_order);
 ...
 const catalogSourceChips=(list:CatalogSourceId[],onToggle:(id:CatalogSourceId)=>void)=><><label>Exercise libraries</label><p className="muted">Choose which exercise databases appear when adding or searching exercises. Essentials + Basic are recommended; enable Full DB only if you need niche movements.</p><div className="focus-muscle-grid equipment-grid">{CATALOG_SOURCES.map((o)=><button type="button" key={o.id} className={`focus-chip${list.includes(o.id)?' active':''}`} onClick={()=>onToggle(o.id)} title={o.description}>{o.label}</button>)}</div>{list.length>0&&<p className="muted">Active: {list.map((id)=>catalogSourceLabel(id)).join(', ')}</p>}</>;
