...
import { buildCatalogFilterOptions, catalogResultMeta, countCatalogMatches, hasCatalogSearchInput, searchCatalog } from '../lib/training/catalogSearch';
...
export default function Page(){
 ...
 const [aiPrompt,setAiPrompt]=useState("I'm a baseball player trying to throw harder and hit harder");
 ...
 async function generateWithAi(){if(!session?.access_token)return alert('Sign in to generate programs.'); if(mode==='team'&&!activeTeam)return alert('Create or join a team first.'); if(mode==='team'&&!canEdit())return alert('Only owner/editors can create team programs.'); const prompt=aiPrompt.trim(); if(prompt.length<8)return alert('Describe your program in a few words (e.g. baseball throw/hit power).'); setAiGenerating(true); setAiSummary(''); try{const res=await fetch('/api/programs/generate',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({prompt,weeks,days,dayTypes,focusMuscles,programName,mode,teamId:mode==='team'?activeTeam?.id:null})}); const data=await res.json().catch(()=>({})); if(!res.ok)throw new Error(data?.error||`Generation failed (${res.status})`); setAiSummary(data.program_summary||''); if(data.program_name)setProgramName(data.program_name); await loadPrograms(); setTrainingSubNav(mode); setAppNav('Training');}catch(e:any){alert(e?.message||'AI program generation failed.');}finally{setAiGenerating(false);}}
 ...
 function openAddExercisePanel(section:string,supersetGroupId?:string|null){if(!canEdit())return; const pending=supersetGroupId||pendingSupersetGroup[section]; const config=pending?{...emptyAddPanelConfig(),mode:'superset' as const,supersetGroupId:pending}:emptyAddPanelConfig(); if(supersetGroupId)setPendingSupersetGroup({...pendingSupersetGroup,[section]:supersetGroupId}); setAddExercisePanel({section,step:'search',query:'',filters:emptyAddPanelFilters(),picked:null,config,custom:emptyAddPanelCustom()});}
...
const weekWorkouts=(program?.st_workouts||[]).filter((w:any)=>w.week===week).sort((a:any,b:any)=>a.day_order-b.day_order);
 ...
 const panelFilterOptions=useMemo(()=>addExercisePanel?buildCatalogFilterOptions(catalog):null,[addExercisePanel,catalog]);
 const panelSearchOpts=addExercisePanel?{query:addExercisePanel.query||'',filters:addExercisePanel.filters||emptyAddPanelFilters(),limit:60}:null;
 const panelMatchCount=panelSearchOpts?countCatalogMatches(catalog,panelSearchOpts):0;
 const panelResults=panelSearchOpts?searchCatalog(catalog,panelSearchOpts):[];
