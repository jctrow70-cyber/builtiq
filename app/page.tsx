...
export default function Page(){
 ...
 async function replaceExerciseWithCatalog(ex:any,catalogItem:any){if(!(await ensurePersonalCopyForTrainingEdit())||!catalogItem)return; const current=planEditRef.current?.workout||workout; if(!current)return; const section=exerciseSection(ex); const payload=catalogPayloadFromItem(catalogItem,section,ex); let updated=0; for(const tw of targetWorkoutsFrom(current)){const match=resolveExerciseTarget(tw,ex,current); if(match){let{error}=await supabase.from('st_exercises').update(payload).eq('id',match.id); if(error&&payload.equipment!==undefined&&/equipment/i.test(error.message||'')){const{equipment:_eq,...rest}=payload; ({error}=await supabase.from('st_exercises').update(rest).eq('id',match.id));} if(error)return alert(error.message); updated++;}} if(!updated)return alert('Could not update that exercise. Try "This workout only" scope.'); await reloadKeepDay();}
