...
export default function Page(){
 ...
 useEffect(()=>{if(appNav==='Training'&&!program)setShowProgramSetup(false);},[appNav,program]);
 useEffect(()=>{
  if(appNav!=='Training')return;
  if(trainingSubNav!=='setup')return;
  if(groupsProgramWizardOpen)return;
  setTrainingSubNav('personal');
  setShowProgramSetup(false);
  setAppNav('Programs');
 },[appNav,trainingSubNav,groupsProgramWizardOpen]);
 useEffect(()=>{
 ...
 async function openDraftForEditing(programId:string,options?:{keepMemberWizard?:boolean}){
  if(!programId||!canEdit())return;
  draftNameSourceRef.current=null;
  if(options?.keepMemberWizard||groupsProgramWizardOpen){
...
function onSelectTrainingDay(date:string){
 ...
 function openManageProgram(){setMemberDashboard(null);setViewingMember(null);setTrainingSessionOpen(false);setTrainingSubNav('personal');setShowProgramSetup(false);setDraftEditProgramId(null);setAppNav('Programs');}
