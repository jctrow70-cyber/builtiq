...
import { MacroTotals, NutritionGoals, formatMacro, formatMacroGrams } from '../../lib/nutrition/macros';
...
export default function NutritionMacroDashboard({ totals, goals }: NutritionMacroDashboardProps) {
  return (
    <div className="nutrition-macro-dashboard" aria-label="Daily macro progress">
      ...
      <NutritionMacroRing
        label="Protein"
        value={`${formatMacroGrams(totals.protein_g)}g`}
        subtitle={`/ ${formatMacroGrams(goals.protein_g)}g`}
        actual={totals.protein_g}
        target={goals.protein_g}
      />
      <NutritionMacroRing
        label="Carbs"
        value={`${formatMacroGrams(totals.carbs_g)}g`}
        subtitle={`/ ${formatMacroGrams(goals.carbs_g)}g`}
        actual={totals.carbs_g}
        target={goals.carbs_g}
      />
      <NutritionMacroRing
        label="Fat"
        value={`${formatMacroGrams(totals.fat_g)}g`}
        subtitle={`/ ${formatMacroGrams(goals.fat_g)}g`}
        actual={totals.fat_g}
        target={goals.fat_g}
      />
    </div>
