#!/usr/bin/env node
/**
 * Fetch ExerciseDB v1 OSS API and convert to BuildIQ import JSON.
 * https://oss.exercisedb.dev/docs — attribution required on free tier.
 *
 * Usage:
 *   npm run import:fetch:exercisedb
 *   npm run import:fetch:exercisedb -- --out scripts/import-exercises/data/exercisedb/buildiq-import.json
 */
...
const DEFAULT_OUT = 'scripts/import-exercises/data/exercisedb/buildiq-import.json';
const PAGE_SIZE = 25;
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--out' && argv[i + 1]) out = argv[++i];
      ...
      console.log(`Fetch ExerciseDB OSS → BuildIQ import format

Options:
  --out <path>       BuildIQ import JSON (default: ${DEFAULT_OUT})
