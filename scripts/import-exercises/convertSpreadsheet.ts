#!/usr/bin/env node
/**
 * Convert Excel/CSV exercise spreadsheet → BuildIQ import JSON.
 * Save Excel as "CSV UTF-8" or use .csv from the template.
 *
 * Usage:
 *   npm run import:convert:spreadsheet -- --file my-exercises.csv
 *   npm run import:convert:spreadsheet -- --file my-exercises.csv --source my_gym --out buildiq.json
 */
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if ((a === '--file' || a === '-f') && argv[i + 1]) file = argv[++i];
      ...
      console.log(`Convert CSV spreadsheet to BuildIQ import JSON

Options:
  --file, -f <path>     CSV file (required). In Excel: File → Save As → CSV UTF-8
  --out, -o <path>      Output JSON path (default: <file>-buildiq.json)
...
function main() {
  ...
  const outPath =
    ...
    path.join(
      path.dirname(abs),
      `${path.basename(abs, path.extname(abs))}-buildiq.json`
    );
