...
$InstallRoot = Join-Path $env:LOCALAPPDATA 'buildiq-node'
$NodeDir = Join-Path $InstallRoot "node-v$NodeVersion-win-x64"
$ZipPath = Join-Path $InstallRoot $ZipName

Write-Host "BuildIQ portable Node (no admin)" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Close and reopen PowerShell, then run buildiq-setup.cmd" -ForegroundColor Yellow
